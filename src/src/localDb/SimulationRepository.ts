...
export async function getSimulationResult(projectId: string, confidence: string): Promise<SimulationResult | null | string> {
    ...
    if (!taskObjectStore || !resources) {
        return "Task Object Store or Resources not found";
    }
    ...
    if (taskObjectStore.tasks.length === 0) {
    ...
    const dataResponse = await axios.post(import.meta.env.VITE_GANTT_BOT_APIS + '/simulation/run', simulationRequest);
